...
class ProductDecoder :
	"""
	A decoder class that handles the decoding work.
	"""
	
	def __init__(self) :
		self.products = __process_rich_signature()
		pass
	...
	@staticmethod
	def __process_rich_signature() :
		with open(GetInputFilePath(), 'rb') as fh :
			# Read the PE header offset from the MZ header.
			fh.seek(0x3C)
			pe_offset = __read_u32(fh)
			print "PE header offset at %08X" % (pe_offset)
			fh.seek(pe_offset)
			if __read_u32(fh) != 0x00004550 :
				print "PE header magic is invalid."
				return
			...
			return product_list
	# End of __process_rich_signature()
